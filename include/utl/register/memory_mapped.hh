...
namespace memory_mapped {
    ...
    template <auto* A, std::remove_pointer_t<decltype(A)> R>
        requires utl::integer::any_unsigned_convertible<std::remove_pointer_t<decltype(A)>>
    ...
    template <auto* A, std::remove_pointer_t<decltype(A)> R>
    constexpr auto tag_invoke(reset_value_t, mm_shared<A,R>)
    {
        return R;
    }

    template <auto* A, std::remove_pointer_t<decltype(A)> R>
    constexpr auto tag_invoke(width_t, mm_shared<A,R>)
    {
        return utl::integer::width(R);
    }
