...
struct printf_logger {
    static utl::result<void> write(utl::string_view const& str) {
        auto* substr = new char[str.size()+1]; //NOLINT(cppcoreguidelines-owning-memory)
        strncpy(substr, str.data(), str.size());
        substr[str.size()] = '\0'; //NOLINT(cppcoreguidelines-pro-bounds-pointer-arithmetic)
        printf("%s",substr);
